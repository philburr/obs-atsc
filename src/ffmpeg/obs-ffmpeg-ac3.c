...
static const char *ac3_getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return obs_module_text("FFmpegAC3");
}
...
static void enc_destroy(void *data)
{
	...
	if (enc->parameters)
		avcodec_parameters_free(&enc->parameters);
	if (enc->aframe)
		...
		const char *type, const char *alt)
{
	...
	enc->parameters = avcodec_parameters_alloc();
	if (enc->parameters)
		avcodec_parameters_from_context(enc->parameters, enc->context);
...
static void *ac3_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	return enc_create(settings, encoder, "ac3", NULL);
}
...
static bool enc_extra_data(void *data, uint8_t **extra_data, size_t *size)
{
	...
	*extra_data = (uint8_t*)enc->parameters;
	*size       = sizeof(*enc->parameters);
	return true;
}
...
struct obs_encoder_info ac3_encoder_info = {
	.id             = "ffmpeg_ac3",
	.type           = OBS_ENCODER_AUDIO,
	.codec          = "ac3",
	.get_name       = ac3_getname,
	.create         = ac3_create,
	.destroy        = enc_destroy,
	.encode         = enc_encode,
